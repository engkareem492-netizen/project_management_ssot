...
// ===========================
// PHASE 1: Sub-Tasks / Task Hierarchy
// ===========================

export async function getChildTasks(parentTaskId: number, projectId: number) {
  const db = await getDb();
  if (!db) return [];
  return await db.select().from(tasks)
    .where(and(eq(tasks.parentTaskId, parentTaskId), eq(tasks.projectId, projectId)))
    .orderBy(tasks.importedAt);
}

export async function getTaskHierarchy(projectId: number) {
  const db = await getDb();
  if (!db) return [];
  const allTasks = await db.select().from(tasks)
    .where(eq(tasks.projectId, projectId))
    .orderBy(tasks.hierarchyLevel, tasks.importedAt);

  // Build tree in application layer
...
export async function moveTask(taskId: number, newParentId: number | null, projectId: number) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");

  let hierarchyLevel = 0;
  if (newParentId) {
    const parent = await db.select().from(tasks).where(eq(tasks.id, newParentId)).limit(1);
    if (parent.length > 0) {
      hierarchyLevel = (parent[0].hierarchyLevel || 0) + 1;
      if (hierarchyLevel > 2) throw new Error("Maximum nesting depth (3 levels) exceeded");
    }
  }

  await db.update(tasks).set({ parentTaskId: newParentId, hierarchyLevel }).where(eq(tasks.id, taskId));

  // Update children hierarchy levels recursively
  const children = await db.select().from(tasks).where(eq(tasks.parentTaskId, taskId));
  for (const child of children) {
    await moveTask(child.id, taskId, projectId);
  }
}
...
export async function getCommentsByEntity(entityType: string, entityId: number, projectId: number) {
  const db = await getDb();
  if (!db) return [];
  const result = await db.select({
    comment: comments,
...
export async function updateComment(id: number, body: string, authorId: number) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
  await db.update(comments).set({ body, isEdited: true }).where(and(eq(comments.id, id), eq(comments.authorId, authorId)));
}
...
export async function getCommentCount(entityType: string, entityId: number) {
  const db = await getDb();
  if (!db) return 0;
...
export async function getAttachmentsByEntity(entityType: string, entityId: number, projectId: number) {
  const db = await getDb();
  if (!db) return [];
  const result = await db.select({
...
export async function deleteAttachment(id: number) {
  ...
  return existing[0]; // return deleted record for storage cleanup
}
...
export async function getTagsByProject(projectId: number) {
  const db = await getDb();
  if (!db) return [];
  return await db.select().from(tags).where(eq(tags.projectId, projectId)).orderBy(tags.name);
}

export async function createTag(data: InsertTag) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
  return await db.insert(tags).values(data);
}

export async function updateTag(id: number, data: Partial<{ name: string; color: string }>) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
...
export async function deleteTag(id: number) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
  // Delete all entity tag associations first
  await db.delete(entityTags).where(eq(entityTags.tagId, id));
  await db.delete(tags).where(eq(tags.id, id));
}

export async function getTagsForEntity(entityType: string, entityId: number) {
  const db = await getDb();
  if (!db) return [];
  const result = await db.select({
...
export async function addTagToEntity(tagId: number, entityType: string, entityId: number) {
  ...
  const existing = await db.select().from(entityTags)
    .where(and(eq(entityTags.tagId, tagId), eq(entityTags.entityType, entityType), eq(entityTags.entityId, entityId)))
    .limit(1);
...
export async function createChecklistItem(data: InsertTaskChecklist) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
  return await db.insert(taskChecklists).values(data);
}

export async function updateChecklistItem(id: number, data: Partial<{ title: string; isCompleted: boolean; sortOrder: number; completedAt: Date | null; completedBy: number | null }>) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
...
export async function getChecklistProgress(taskId: number) {
  const db = await getDb();
  if (!db) return { completed: 0, total: 0, percentage: 0 };
  const items = await db.select().from(taskChecklists).where(eq(taskChecklists.taskId, taskId));
...
export async function globalSearch(projectId: number, query: string, limitPerType = 5) {
  const db = await getDb();
  if (!db) return [];

  const searchPattern = `%${query}%`;
  const results: Array<{ entityType: string; entityId: number; idCode: string; title: string; matchField: string }> = [];

  // Search requirements
  const reqs = await db.select().from(requirements)
    .where(and(eq(requirements.projectId, projectId), or(
      like(requirements.idCode, searchPattern),
      like(requirements.description, searchPattern),
    )))
    .limit(limitPerType);
  for (const r of reqs) {
    results.push({ entityType: 'requirement', entityId: r.id, idCode: r.idCode, title: r.description?.substring(0, 100) || '', matchField: 'description' });
  }

  // Search tasks
  const tks = await db.select().from(tasks)
    .where(and(eq(tasks.projectId, projectId), or(
      like(tasks.taskId, searchPattern),
      like(tasks.description, searchPattern),
    )))
    .limit(limitPerType);
  for (const t of tks) {
    results.push({ entityType: 'task', entityId: t.id, idCode: t.taskId, title: t.description?.substring(0, 100) || '', matchField: 'description' });
  }

  // Search issues
  const iss = await db.select().from(issues)
    .where(and(eq(issues.projectId, projectId), or(
      like(issues.issueId, searchPattern),
      like(issues.description, searchPattern),
    )))
    .limit(limitPerType);
  for (const i of iss) {
    results.push({ entityType: 'issue', entityId: i.id, idCode: i.issueId, title: i.description?.substring(0, 100) || '', matchField: 'description' });
  }

  // Search risks
  const rks = await db.select().from(risks)
    .where(and(eq(risks.projectId, projectId), or(
      like(risks.riskId, searchPattern),
      like(risks.title, searchPattern),
    )))
    .limit(limitPerType);
  for (const r of rks) {
    results.push({ entityType: 'risk', entityId: r.id, idCode: r.riskId, title: r.title, matchField: 'title' });
  }

  // Search knowledge base
  const kbs = await db.select().from(knowledgeBase)
    .where(and(eq(knowledgeBase.projectId, projectId), or(
      like(knowledgeBase.code, searchPattern),
      like(knowledgeBase.title, searchPattern),
      like(knowledgeBase.description, searchPattern),
    )))
    .limit(limitPerType);
  for (const k of kbs) {
    results.push({ entityType: 'knowledgeBase', entityId: k.id, idCode: k.code || '', title: k.title || '', matchField: 'title' });
  }

  // Search assumptions
  const asms = await db.select().from(assumptions)
    .where(and(eq(assumptions.projectId, projectId), or(
      like(assumptions.assumptionId, searchPattern),
      like(assumptions.description, searchPattern),
    )))
    .limit(limitPerType);
  for (const a of asms) {
    results.push({ entityType: 'assumption', entityId: a.id, idCode: a.assumptionId, title: a.description?.substring(0, 100) || '', matchField: 'description' });
  }

  // Search dependencies
  const deps = await db.select().from(dependencies)
    .where(and(eq(dependencies.projectId, projectId), or(
      like(dependencies.dependencyId, searchPattern),
      like(dependencies.description, searchPattern),
    )))
    .limit(limitPerType);
  for (const d of deps) {
    results.push({ entityType: 'dependency', entityId: d.id, idCode: d.dependencyId, title: d.description?.substring(0, 100) || '', matchField: 'description' });
  }
