...
export const tasks = mysqlTable("tasks", {
  ...
  lastUpdate: text("lastUpdate"),
  updateDate: varchar("updateDate", { length: 50 }),
  parentTaskId: int("parentTaskId"),          // self-referential: null = top-level task
  hierarchyLevel: int("hierarchyLevel").default(0).notNull(), // 0=root, 1=child, 2=grandchild (max 3 levels)
  importedAt: timestamp("importedAt").defaultNow().notNull(),
  updatedAt: timestamp("updatedAt").defaultNow().onUpdateNow().notNull(),
});
...
/**
 * Comments table - polymorphic comments on any entity
 */
...
/**
 * Attachments table - polymorphic file attachments on any entity
 */
...
/**
 * Tags table - project-scoped tags/labels
 */
...
/**
 * Entity Tags - many-to-many linking tags to any entity
 */
export const entityTags = mysqlTable("entityTags", {
  id: int("id").autoincrement().primaryKey(),
...
/**
 * Task Checklists - checklist items within tasks
 */
